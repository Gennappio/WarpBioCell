...
def bench_grid(nodes, n_cells, device, repeats, warmup):
    ...
    def sweep():
        for color in (0, 1):
            wp.launch(sor_sweep, dim=geom.shape, inputs=[color, 1.8, dx2_over_D, kin.uptake_max, kin.michaelis_k, *bc, ox.density, ox.values], device=device)

    def residual():
        ox._residual.zero_()
        wp.launch(steady_state_residual, dim=geom.shape, inputs=[dx2_over_D, kin.uptake_max, kin.michaelis_k, 1.0 / params.boundary_value, *bc, ox.density, ox.values, ox._residual], device=device)
