...
def _run_one_pass(positions, radii, params, device):
    ...
    # dt = 0 leaves positions untouched, so neighbor_count reflects the input configuration.
    contact_substep(pop, grid, params, dt=0.0)
    return pop


def test_neighbor_counts_match_brute_force(cpu):
    r = 8.0
    ...
    _, expected = contact_velocities_reference(pos, radii, params.rate, params.query_radius)

    np.testing.assert_array_equal(pop.neighbor_counts_numpy(), expected)
    assert expected.max() >= 6  # the configuration is actually crowded


def test_neighbor_counts_with_mixed_radii(cpu):
...
def test_isolated_cells_have_no_contacts(cpu):
    ...
    assert np.all(pop.neighbor_counts_numpy() == 0)
...
def test_grid_only_indexes_active_prefix(cpu):
    ...
    np.testing.assert_array_equal(pop.neighbor_counts_numpy(), [1, 1])
